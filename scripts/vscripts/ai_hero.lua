...
function HeroAI:IsValidPosition(pos)
    ...
    if pos.y == nil or pos.x == nil or pos.y < 2048 or pos.y > 3600 or pos.x > 5600 or pos.x < -5600 then
        return false
    end
...
function HeroAI:IsAlive(target)
    ...
    if(target.HasModifier == nil or target:HasModifier("modifier_hero_waitting")) then
        return false
    end
...
function HeroAI:OnHeroThink(hero)
    ...
    if(hero.IsManuallyControlled == true) then
        return nil
    end
    ...
    if(GameRules:GetGameTime() - GameRules.DW.StageStartTime > 5) then
        ...
        local needAdjustPosition = true
        if(hero:GetName() == "npc_dota_hero_monkey_king") then
            if(hero:HasModifier("modifier_monkey_king_tree_dance_hidden") or hero:HasModifier("modifier_monkey_king_tree_dance_activity")) then
        ...
        if(needAdjustPosition and heroPosition.y < 2050 or heroPosition.y > 3600 or heroPosition.x < -5600 or heroPosition.x > 5600) then
            local newPos = Vector(heroPosition.x, heroPosition.y, heroPosition.z)
            if(heroPosition.x > 5600) then
                newPos.x = 5600
            end
            ...
            if(heroPosition.y > 3600) then
                newPos.y = 3600
            end

            if(heroPosition.y < 2050) then
                newPos.y = 2050
            end
    ...
    if(hero.IsOutOfGame ~= nil and hero:IsOutOfGame()) then
        if(hero:GetName() ~= "npc_dota_hero_phoenix") then
            ...
            if(hero:HasModifier("modifier_phoenix_supernova_hiding") == false or GameRules.DW.CheckHasTalent(hero, "special_bonus_unique_phoenix_7") == false) then
                return 0.2
            end
        end
    end
...
function HeroAI:EvaluateCommand(hero, cmdName)
    ...
    if(cmdName == "ATTACK_TARGET") then
        ...
        if(hero.HasItemInInventory ~= nil and hero:HasItemInInventory("item_ai_delay")) then
            if(GameRules:GetGameTime() - GameRules.DW.StageStartTime < 8) then
                return 0, nil
            end
        end
            ...
            if(heroPos.x > -4300 and heroPos.x < 4300) then
                if(hero.HasItemInInventory ~= nil and hero:HasItemInInventory("item_no_attack")) then
                    if(hero:GetName() ~= "npc_dota_hero_meepo" or hero:GetHealthPercent() < 50) then
                        nearestEnemy = HeroAI:GetClosestEnemyHero(hero, FIND_UNITS_EVERYWHERE)
                    end
                end
            end
    ...
    if(cmdName == "USE_ABILITY") then
        ...
        if(GameRules:GetGameTime() - GameRules.DW.StageStartTime < delayTime) then
            return 0, nil
        end
    ...
    if(cmdName == "MOVE_TO_POSITION") then
        ...
        if(hero:GetName() == "npc_dota_hero_batrider") then
            if(hero:HasModifier("modifier_batrider_flaming_lasso_self")) then
                local vTargetLoc = GameRules.DW.FountainGood
                if(hero:GetTeamNumber() ~= DOTA_TEAM_GOODGUYS) then
                    vTargetLoc = GameRules.DW.FountainBad
                end
...
function HeroAI:ExecuteCommand(hero, cmdName, cmdData)
    ...
    if(cmdName == "ATTACK_TARGET") then
        if(cmdData == nil or cmdData:IsNull()) then
            hero:MoveToPositionAggressive(GameRules.DW.BattleFightPosition)
            return 0.2
        end
            ...
            if(phantomRush ~= nil and phantomRush:GetLevel() > 0) then
                if(hero:IsDisarmed() == false) then
                    if(hero:HasScepter() or phantomRush:IsCooldownReady()) then
                        ...
                        if(GameRules.DW.CheckHasTalent(hero, "special_bonus_unique_phantom_lancer")) then
        ...
        elseif(isLandMine) then
            ...
            if(GameRules:GetGameTime() - GameRules.DW.StageStartTime < 10) then
                local heroPosition = hero:GetAbsOrigin()
                if(heroPosition.x < -500 or heroPosition.x > 500) then
                    targetPosition.y = heroPosition.y
                end
            end

            if(HeroAI:IsValidPosition(targetPosition) == false) then
                targetPosition = GameRules.DW.BattleFightPosition
            end
...
function HeroAI:GetSpellData(hSpell)
    ...
    if(abilityName == "furion_teleportation" or abilityName == "abyssal_underlord_dark_rift") then
        if(hero:HasModifier("modifier_batrider_flaming_lasso_self") or hero:HasModifier("modifier_nyx_assassin_burrow")) then
            return nil
        end

        if(hero:GetHealthPercent() > 50 and hero:GetManaPercent() > 20) then
            return nil
        else
            if(hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS) then
                return {ability = hSpell, type = "point_target", target = GameRules.DW.FountainGood}
    ...
    if(abilityName == "wisp_relocate") then
        ...
        if(hero.tetherTarget ~= nil and HeroAI:IsAlive(hero.tetherTarget) and hero.tetherTarget:GetHealthPercent() < 40) then
            if(hero.tetherTarget:HasModifier("modifier_wisp_tether_haste")) then
                return {ability = hSpell, type = "point_target", target = vTargetLoc}
            end
        end
    ...
    if(abilityName == "enchantress_bunny_hop") then
        ...
        if(heroPos.x < -4300 or heroPos.x > 4300) then
            return nil
        end
    ...
    if(abilityName == "techies_minefield_sign") then
        local spawnLocation = GameRules.DW.BattleFightPosition
        local heroLocation = hero:GetAbsOrigin()
        if(hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS) then
            spawnLocation = Vector(spawnLocation.x - 1800, heroLocation.y, heroLocation.z)
        else
            spawnLocation = Vector(spawnLocation.x + 1800, heroLocation.y, heroLocation.z)
        end

        local wayFromCenter = (heroLocation - spawnLocation):Length2D()
        if(wayFromCenter <= 450) then
            return {ability = hSpell, type = "point_target", target = spawnLocation}
        end
    ...
    if(abilityName == "void_spirit_aether_remnant") then
        if(HeroAI:HasInvisibleEnemyNearby(hero, 600)) then
            if(GameRules.DW.CheckHasTalent(hero, "special_bonus_unique_void_spirit_7")) then
                return {ability = hSpell, type = "point_target", target = hero:GetAbsOrigin()}
            end
        end
    ...
    if(abilityName == "shredder_timber_chain" or abilityName == "morphling_waveform" or abilityName == "shredder_chakram" or abilityName == "shredder_chakram_2" or abilityName == "tusk_frozen_sigil"
        ...
        if(abilityName == "morphling_waveform" and hero:GetHealthPercent() > 50) then
            if(GameRules:GetGameTime() - GameRules.DW.StageStartTime < 20) then
                return nil
            end
        end
    ...
    if(abilityName == "templar_assassin_trap_teleport") then
        if(GameRules:GetGameTime() - GameRules.DW.StageStartTime < 30) then
            return nil
        end
    ...
    if(abilityName == "storm_spirit_ball_lightning") then
        ...
        if(hero:GetManaPercent() < 30 or hero:GetHealthPercent() < 30) then
            if(hero:HasModifier("modifier_fountain_aura_buff") == false) then
                return {ability = hSpell, type = "point_target", target = fountainLoc}
            end
        end
        ...
        if(hTarget ~= nil and hTarget:IsNull() == false) then
            ...
            if(hero:GetMana() < manaCostEstimate + manaEscape and hero:GetManaPercent() < 50) then
                if(hero:HasModifier("modifier_fountain_aura_buff") == false) then
                    return {ability = hSpell, type = "point_target", target = fountainLoc}
                end
            end
    ...
    if(abilityName == "broodmother_spawn_spiderlings" and hero:GetHealthPercent() > 45) then
        ...
        if(GameRules.DW.CheckHasTalent(hero, "special_bonus_unique_broodmother_3")) then
            damage = damage + 125
        end
    ...
    if(abilityName == "queenofpain_blink") then
        ...
        if(HeroAI:HasEnemyNearby(hero, 500) and isUltiCd) then
            ...
            local castVector = (vTargetLoc - hero:GetAbsOrigin()):Normalized()
            local castPos = hero:GetAbsOrigin() + castVector * 1000
            
            if castPos ~= nil and HeroAI:IsValidPosition(castPos) then
                return {ability = hSpell, type = "point_target", target = castPos}
            end
        else
...
function HeroAI:IsNoTargetSpellCastValid(hSpell, targetTeamType)
    ...
    if(abilityName == "venomancer_poison_nova") then
        if(GameRules.DW.CheckHasTalent(caster, "special_bonus_unique_venomancer_6")) then
            nAbilityRadius = 1500
        end
    end
