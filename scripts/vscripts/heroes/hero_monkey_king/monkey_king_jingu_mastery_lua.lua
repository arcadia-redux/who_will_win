...
function modifier_monkey_king_jingu_mastery_lua:OnAttackLanded( keys )
	...
	if jinguMasteryStack_debuff:GetStackCount() + 1 >= self.required_hit then
		...
		self.caster:AddNewModifier(self.caster, self.ability, "modifier_monkey_king_jingu_mastery_lua_buff", {duration = self.max_duration})
		
		local jinguBuffStack = self.caster:FindModifierByName("modifier_monkey_king_jingu_mastery_lua_buff")
...
function modifier_monkey_king_jingu_mastery_lua_buff:GetModifierPreAttack_BonusDamage(params)
	if self:GetParent():FindModifierByName("special_bonus_unique_monkey_king_2") then
       local hTalent = self:GetParent():FindModifierByName("special_bonus_unique_monkey_king_2")
       if hTalent and hTalent:GetLevel()>1 then
...
function modifier_monkey_king_jingu_mastery_lua_buff:OnAttackLanded( keys )
	local attacker 		= keys.attacker
	local caster 				= self:GetCaster()
	local target 				= keys.target
	local ability 				= keys.ability
