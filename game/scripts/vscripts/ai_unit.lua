...
function UnitAI:OnUnitThink(unit)
    ...
    --print(highestScore, UNIT_CMD_LIST[highestScoreCommand], highestData)
...
function UnitAI:IsInBattleFightArea(hero)
    ...
    hero, 800, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, UNIT_FILTER, 0, true)
...
function UnitAI:ExecuteCommand(unit, cmdName, cmdData)
    if(cmdName == "ATTACK_TARGET") then
        if(cmdData == nil) then
            unit:MoveToPositionAggressive(unit.targetPoint)
            return 1
        end
        ...
        if(unit:GetAttackDamage() > 1 or unitName == "npc_dota_techies_stasis_trap" or unitName == "npc_dota_techies_land_mine" or unitName == "npc_dota_techies_remote_mine" or unitName == "npc_dota_grimstroke_ink_creature") then
            ...
            local team = unit:GetTeamNumber()
            local moveVector = unit.targetPoint
...
function UnitAI:ClosestEnemyAll(unit, teamId)
    ...
    for index = 1, #enemies do
        if(enemies[index]:IsAlive() and enemies[index]:IsInvulnerable() == false and enemies[index]:IsAttackImmune() == false) then
...
function UnitAI:GetBestHeroTargetInRange(hSpell)
    ...
    local enemies = FindUnitsInRadius(teamId, unit:GetAbsOrigin(), unit, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP,
    UNIT_FILTER, FIND_CLOSEST, true)
...
function UnitAI:GetBestFriendlyTarget(hSpell)
    ...
    UnitAI:GetSpellRange(hSpell), DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, UNIT_FILTER, 0, true)
...
function UnitAI:GetNearestWeekestFriendlyTarget(unit)
    ...
    1500, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, UNIT_FILTER, 0, true)
...
function UnitAI:IsNoTargetSpellCastValid(hSpell, targetTeamType)
    ...
    hSpell:GetCaster(), nAbilityRadius, targetTeamType, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, UNIT_FILTER, 0, true)
...
function UnitAI:FindClearSpaceToMove(hero)
    ...
    local units = FindUnitsInRadius(hero:GetTeamNumber(), hero:GetAbsOrigin(),
    hero, checkRadius, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, UNIT_FILTER, 0, true)
